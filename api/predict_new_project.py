...
def predict_new_project(payload: NewProjectInput):
    ...
    if payload.email and payload.email.strip():
        email_subject = (
            f"Land Acquisition Delay Risk Prediction – {payload.project_id}"
        )

        email_message = f"""
LAND ACQUISITION DELAY RISK PREDICTION

Project ID: {payload.project_id}

Risk Tier: {risk_tier}
Delay Probability: {prediction_result["predicted_delay_probability"]:.1f}%
Priority Score: {priority_score:.2f}

TOP RISK DRIVERS
{chr(10).join(f"- {driver}" for driver in driver_labels)}

RECOMMENDED ACTIONS
{chr(10).join(f"- {action}" for action in list(dict.fromkeys(recommended_actions)))}

NOTE
This prediction was generated by the project's LightGBM delay-risk model.
SHAP-based explanation was used where available.
""".strip()
